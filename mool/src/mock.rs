...
use crate::{DBSession, Database, DbError, Row, Statement};
...
impl MockDBSession {
    ...
    fn take_next(
        ...
        let next = self.planned.pop_front().ok_or_else(|| {
            if self.strict {
                panic!(
                    "MockDbSession: unexpected call {:?} with SQL: {:?}",
                    kind, stmt
                );
            }
...
impl DBSession for MockDBSession {
    async fn execute(&mut self, qs: Statement) -> Result<u64, DbError> {
        ...
        match self.take_next(DbCallKind::Execute, &qs)? {
            ...
            other => panic!(
                "MockDbSession: wrong planned response for execute: {:?}",
