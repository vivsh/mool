...
let schema = db::schema()
    .model::<PostWithStatus>()
    .extend_table("posts", |table| table.unmanaged_prefix("UNLOGGED"))
    .opaque("CREATE INDEX posts_live_idx ON posts (id) WHERE published")
    .build()?;
```

Dialect-specific parent-table syntax uses the same extension point. For
example, PostgreSQL `PARTITION BY RANGE (purge_at)` belongs in an unmanaged
table suffix:

```rust
let schema = db::schema()
    .model::<CrawlerUrl>()
    .extend_table("crawler_url", |table| {
        table.unmanaged_suffix("PARTITION BY RANGE (purge_at)")
    })
    .build()?;
```

Rolling child partitions are operational objects and are not modeled or
generated by Mool migrations. Converting an existing plain table requires an
explicit reviewed raw migration.
